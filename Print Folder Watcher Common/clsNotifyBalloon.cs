...
{
	// We have http://www.codeproject.com/cs/miscctrl/notifyiconex.asp
	// to thank for this code. It's not pretty but seems to work.

	// Here are a few comments from this web site:

	// Known issues
	// I must admit that the documentation concerning the balloon tip does not seem to match the behaviour. 
	// The timeout value does not seem to work. I could not get the correct windows message when the balloon 
	// is closed by the 'x' button. I get the 'timeout' message whether the balloon times out or it is closed 
	// by the user. 
	//
	// Watch Out
	// I must warn you that if you use my NotifyIconEx class at the same time as NotifyIcon strange 
	// things may happen. See if you can figure out why

	public class clsNotifyBalloon : System.ComponentModel.Component
	{
		...
		public void ShowBalloon(string title, string text, NotifyInfoFlags type)
		{
			int timeoutInMilliSeconds = 10;	// Doesn't seem to work.
